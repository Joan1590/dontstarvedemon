...
local GeometricOptionsScreen = Class(Screen, function(self, modname, colorname_vectors, outlined_anims)
	...
	self.bg = self.proot:AddChild(TEMPLATES.RectangleWindow(619, 359, "几何布局设置", close_button))
    ...
    self.subtitle_geometry:SetString("形状")
    ...
    self.subtitle_color_good:SetString("开放")
    ...
    self.subtitle_color_bad:SetString("阻塞")
    ...
    self.subtitle_misc:SetString("其他配置调整")
    ...
    self.subtitle_gridsize:SetString("网格密度大小配置")
    ...
    self.subtitle_gridsize1:SetString("种植/放置")
    ...
    self.subtitle_gridsize2:SetString("墙")
	...
	local placer_color_options = {
		...
		{description = "蓝色", data = "blue", hover = "如果你是红/绿色盲，蓝色很有用."},
		{description = "红色", data = "red", hover = "游戏使用的普通红色."},
		{description = "白色", data = "white", hover = "明亮的白色，能见度更好."},
		{description = "黑色", data = "black", hover = "黑色与较亮的颜色形成对比."},
	}
	local color_options = {}
	for i,option in ipairs(placer_color_options) do
		color_options[i] = option
	end
	color_options[#color_options+1] = {description = "白色轮廓", data = "whiteoutline", hover = "白色与黑色的轮廓，为最好的能见度."}
	color_options[#color_options+1] = {description = "黑色轮廓", data = "blackoutline", hover = "黑色与白色的轮廓，为最好的能见度."}
	local hidden_option = {description = "隐藏的", data = "hidden", hover = "完全隐藏它，因为你根本不需要看到它"}
	...
	local color_type_descriptions = {"精细/墙:", "", "地皮:", "", "乐趣:", "", "地皮颜色:"}
	local color_type_hovers = {
		_GOOD = "没有阻塞，你可以把它放在那里.",
		_BAD = "被阻塞了，你不能把它放在那里.",
		[""] = "网格点的颜色 ",
		TILE = "草坪瓷砖的颜色 ",
		PLACER = "对象的卷影副本的颜色 ",
		NEARTILE = "放置物最近的地皮和墙砖的轮廓颜色.",
	}
	...
	self.color_buttons = {
		redgreen = { text = "红/绿", hover = "普通游戏使用的标准红色和绿色.", goodcolor = "green", badcolor = "red"},
		redblue = { text = "红/蓝", hover = "用蓝色代替绿色，对红/绿色盲没有帮助.", goodcolor = "blue", badcolor = "red"},
		blackwhite = { text = "黑/白", hover = "黑色表示阻塞，白色表示可替换，通常更显眼.", goodcolor = "white", badcolor = "black"},
		blackwhiteoutline = { text = "轮廓", hover = "黑色和白色，但与轮廓，以提高能见度.", goodcolor = "whiteoutline", badcolor = "blackoutline"},
		custom = { text = "定制", hover = "自定义每种类型的点\n具有自己的颜色或隐藏."},
		preset = { text = "预设", hover = "切换回预设选择模式，\n允许您快速选择配色方案."},
	}
    local button_y = 30
	...
	local geometries = {
		{name="Square", hover="正方形."},
		{name="Diamond", hover="菱形."},
		{name="X Hexagon", hover="六角形，与X轴平行."},
		{name="Flat Hexagon", hover="横六角形."},
		{name="Z Hexagon", hover="六角形，与Z轴平行."},
		{name="Pointy Hexagon", hover="竖六边形."},
	}
	...
	local toggle_strings = {[true] = "关闭mod.",
							[false]= "打开mod."}
	...
	local toggle_buttons = {
		{name="grid", hover="是否显示种植放置网格."},
		{name="placer", hover="是否显示放置物虚影"},
		{name="cursor", hover="是否在鼠标显示放置物.", toggle=2, atlases={"", "_num", ""}},
		{name="smart_spacing", hover="是否自动调整网格间距."},
		{name="till_grid", hover="是否使用网格模式耕作农田."},
	}

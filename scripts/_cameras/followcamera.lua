...
local FollowCamera = Class(function(self, inst)
    ...
    self.automove_dir = 0
    self.automove_speed = 0
    self.automove = true
    self.automove_delay = 0

    self.distance_override = nil
    self.pitch_override = nil
...
function FollowCamera:Apply()
    if self.pitch_override then 
        self.pitch = self.pitch_override
    end
    if self.heading_override then
        self.heading = self.heading_override
    end
    if self.distance_override then 
        self.distance = self.distance_override
    end
    --dir
...
function FollowCamera:ZoomIn(step)
    ...
    if self.free_camera then 
        self.distancetarget = self.distancetarget - (step or self.zoomstep)
    end
end
...
function FollowCamera:Update(dt)
    ...
    if self.cutscene then
        ...
        if self.time_since_zoom ~= nil and not self.cutscene then
            ...
            if self.should_push_down and self.time_since_zoom > 0.25 and not self.free_camera and not self.free_distance then
    ...
    elseif self.currentscreenxoffset ~= 0 then
        ...
        if math.abs(self.currentscreenxoffset) < 0.01 then
            self.currentscreenxoffset = 0
        end
    end

...
ENV.AddClassPostConstruct("widgets/redux/modstab", function(self)

    self.next_needs_update_mod_button = TEMPLATES.IconButton("images/ui.xml", "arrow2_right.tex", "Go Next Have Update Mod", false, false, function() gonext_fn(self) end, hovertext_top)
    ...
    self.UpdateModsOrder = function(self, force_refresh, ...)
        ...
        if self.downloading_mods_count > 0 then
            if self.workshopupdatetask == nil then
                self.workshopupdatetask = scheduler:ExecutePeriodic( 3, self.UpdateForWorkshop, nil, 0, "updateforworkshop", self)
            end
    ...
    local setstring_hook = function(num)
        if num > 0 then
    ...
    self.out_of_date_badge.count.SetString = function(self, str, ...)
        setstring_hook(tonumber(str))
        return num_setstring(self, str, ...)
    end

    local OnRawKey = self.OnRawKey
    self.OnRawKey = function(self, key, down, ...)
        if down and key == KEY_F5 then
    ...
    self.modfilterbar.search_box.textbox:SetPassControlToScreen(CONTROL_SCROLLBACK, true)
    self.modfilterbar.search_box.textbox:SetPassControlToScreen(CONTROL_SCROLLFWD, true)

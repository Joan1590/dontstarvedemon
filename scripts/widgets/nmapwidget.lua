...
local function NMapWidget(MapWidget)
	MapWidget.offset = Vector3(0,0,0)
	MapWidget.dragpos = Vector3(0,0,0)
	...
	function MapWidget:OnUpdate(dt)
		...
		if self.dragpos and self.lastpos then
			local ox = self.dragpos.x - self.lastpos.x
			local oy = self.dragpos.y - self.lastpos.y

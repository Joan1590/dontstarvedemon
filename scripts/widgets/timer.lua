...
local text_size = tonumber(TUNING.xiaoyao("textsize") or "16")
--local scalesize = tonumber(TUNING.xiaoyao("scalesize") or "1")
local scaletext = TUNING.xiaoyao("scaletext")
local hidecdup = TUNING.xiaoyao("hidecdup")
local colour_attack = TUNING.xiaoyao("colour_attack")
local colour_cooldown = TUNING.xiaoyao("colour_cooldown")
...
local colour_danger = colours[TUNING.xiaoyao("colour_danger")]--Shouldn't crash with nil values, I think.
local colour_warning = colours[TUNING.xiaoyao("colour_warning")]
local colour_safe = colours[TUNING.xiaoyao("colour_safe")]
...
local function GetNearbyCountOfEnt(prefab)--Not too keen on looking through all ents as it might cause performance issues, but I do need to keep track of some entity count like spore trees and shadow hands.
	...
	for k,v in pairs(TheSim:FindEntities(pos.x,0,pos.z,70,{},{"INLIMBO","FX"})) do
...
local function GetOffCooldownTriggerWithMissingEnt(prefab) --This condition should trigger when the entity goes missing, aka dies. If we want to know if there aren't any of those entities, then we just use the function GetNearbyCountOfEnt.
	...
	if GetNearbyCountOfEnt(prefab) == 1 or fw_minion_cond then
		for _,ent in pairs(TheSim:FindEntities(pos.x,0,pos.z,70,{"_combat","_health"},{"INLIMBO","fx"})) do
...
function Timer:AddStageIfCondition(self,condition,prefab,cooldown)
	if (CheckTag(self.owner,condition) or (CheckAnimState(self.owner,condition) and self.owner.AnimState:GetCurrentAnimationTime() <= tick_rate)) and not self.stage_cooldown then
...
function Timer:ResetStageIfCondition(self,condition,prefab)
	if (CheckTag(self.owner,condition) or (CheckAnimState(self.owner,condition) and self.owner.AnimState:GetCurrentAnimationTime() <= tick_rate)) then
		if (self.owner.prefab == prefab) or not prefab then --To either trigger on specific entities or just the anim.
			self.stage = 0
			self.stage_cooldown = false
		end
...
function Timer:CheckMobTable(entity,self)
	...
	local Anims = function(anims)
		return SearchForAnim(anims,entity,self)
	end
	...
	local mob_table = {
		...
		["toadstool"] = anim and Anims("attack_basic"),
		["toadstool_dark"] = anim and Anims("attack_basic"),
...
function Timer:SpecialTuneCooldown(cooldown,prefab,ent,self)
	...
	--[[elseif prefab == "antlion" then
		if self and ent and self.owner.min_attack_period then
			if ent.AnimState and ent.AnimState:IsCurrentAnimation("cast_pre") and (ent.AnimState:GetCurrentAnimationTime() <= TheSim:GetTickTime()) then
				self.min_attack_period = math.max(TUNING.ANTLION_MIN_ATTACK_PERIOD,self.min_attack_period+TUNING.ANTLION_SPEED_UP)
			elseif ent.AnimState and ent.AnimState:IsCurrentAnimation("eat") and (ent.AnimState:GetCurrentAnimationTime() <= TheSim:GetTickTime()) then
				self.min_attack_period = math.min(TUNING.ANTLION_MAX_ATTACK_PERIOD,self.min_attack_period+TUNING.ANTLION_SLOW_DOWN)
			end
		end]]
	end
end
...
function Timer:CheckSpecialAttackChangeConditions(self)
	...
	if not valid_prefab then return nil end--What else am I supposed to do if it's a prefab that doesn't have any special condition?
...
function Timer:UpdateAttackTime()
	...
	if self:CheckMobTable(self.owner,self) and ((anim_time <= tick_rate) or self.will_attack_again - current_time <= 0) then--if (Mob in attack animation) and ((It's the first frame of the attack animation) or (Mob should definitely be able to attack)) then [...]
		self.last_attack_time = current_time-tick_rate
		self.will_attack_again = self.last_attack_time + (self.min_attack_period or 0)
	...
	if self.will_attack_again - current_time >= 0 then
		self.text:SetString(string.format("攻击: %.2fs",self.will_attack_again - current_time))
		self.text:Show()
	elseif self.will_attack_again - current_time < 0 then
		self.text:SetString("攻击: 准备就绪")
		if hidecdup then
			self.text:Hide()
		end
	end
...
function Timer:UpdateCooldownTimes()
	if self.int_timers then
		for k,v in pairs(self.cooldown_text) do 
			...
			local _time = tonumber(self[name.."_time"]) -- When the cooldown was triggered(Time taken via GetTime function)
			local _again = tonumber(self[name.."_again"]) -- When it should trigger again(_time + cooldown time)
			local _left = tonumber(self[name.."_left"]) -- How much time is left for cooldown to be over(_again - current_time)
			...
			if info and info.condition and not (off_cooldown_able) and (is_anim or (is_tag and not (cooldown_over) or other_conditions_trigger)) then
				...
				self[name.."_time"] = current_time; _time = tonumber(self[name.."_time"])-- _time variable won't change if I change the self[name.."_time"] variabl.
				self[name.."_again"] = (is_anim and (_time + info.time - tick_rate)) or (_time + info.time); _again = tonumber(self[name.."_again"])
			end
			if info and info.condition and info.constant and not is_tag and not is_anim then
				self[name.."_time"] = current_time; _time = tonumber(self[name.."_time"])
				self[name.."_again"] = self[name.."_time"] + info.time; _again = tonumber(self[name.."_again"])
			end
			
			self[name.."_left"] = string.format("%.2f",_again - current_time); _left = tonumber(self[name.."_left"])
			local text = ""
			if _left < 0 then
				text = info.name..": ".."准备就绪"
				
				if info.alwayshidden or hidecdup then
			...
			elseif _left >= 0 then
				text = info.name..": "..self[name.."_left"].."s"
				if (info.alwayshidden or hidecdup) and ((_left) > (info.time-tick_rate)) then
					self[name].shown = false
				else
					self[name].shown = true
				end
...
function Timer:ApplyColourBasedOnTime(cd,time,total_time,attack)
	...
	if player_speed and attack_range and attack then--Just for the basic attack, inform player of when they have to run.
		local dodge_time = attack_range/player_speed
...
function Timer:OnUpdate(dt)
	...
	local anim = self.owner.AnimState
		if self.int_timers then
			self:BossStageCheck()
			self:UpdateCooldownTimes()

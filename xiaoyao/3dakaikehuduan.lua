	...
	local KEY = TUNING.xiaoyao("anjian")
	...
	GLOBAL.TheInput:AddKeyUpHandler(
		KEY, 
		function()
			if not GLOBAL.IsPaused() and IsDefaultScreen() and not GLOBAL.TheShard:IsSecondary() then --GLOBAL.ThePlayer.player_classified._isSecondaryShard:value() then --and not GLOBAL.ModIndex:IsModEnabled(ingameClientModManagerServerVersion) then
				local ms = ModsScreen()
	...
	local function ApplyToGame(modsscreen)
		...
		modsscreen.Apply = function(self)
			if GLOBAL.TheWorld ~= nil and GLOBAL.ThePlayer ~= nil then --and not GLOBAL.ModIndex:IsModEnabled(ingameClientModManagerServerVersion) then
	...
	function OverrideDuplicateConnectionDialog(popupdialog)
		if popupdialog.dialog and popupdialog.dialog.title:GetString() == GLOBAL.STRINGS.UI.NETWORKDISCONNECT.TITLE.ID_ALREADY_CONNECTED and GLOBAL.ThePlayer and GLOBAL.ThePlayer.isApplyingModChanges then --and not GLOBAL.ModIndex:IsModEnabled(ingameClientModManagerServerVersion) then
	...
	AddPrefabPostInit("player_classified", function(inst)
		--if not GLOBAL.ModIndex:IsModEnabled(ingameClientModManagerServerVersion) then

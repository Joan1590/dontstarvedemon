...
local language               = TUNING.xiaoyao("LANGUAGE")
local visible_animation      = TUNING.xiaoyao("VISIBLE_ANIMATION")
local timer                  = TUNING.xiaoyao("TIMER")
local phase_name             = TUNING.xiaoyao("PHASE_NAME")
local horizontal_alignment   = TUNING.xiaoyao("HORIZONTAL_ALIGNMENT")
local vertical_alignment     = TUNING.xiaoyao("VERTICAL_ALIGNMENT")
local horizontal_margin      = TUNING.xiaoyao("HORIZONTAL_MARGIN")
local horizontal_margin      = TUNING.xiaoyao("洞穴钟水平边距")
local vertical_margin        = TUNING.xiaoyao("洞穴钟垂直边距")
local scale                  = TUNING.xiaoyao("洞穴钟比例大小")
local visibly_have_medallion = false

modimport("scripts/languages/"..language..".lua")
...
local function AddNightmarePhaseIndicator(self)
    if _G.TheWorld.net.components.nightmareclock ~= nil then

...
local show_widget = TUNING.xiaoyao("show_widget")
local colour_attack = TUNING.xiaoyao("colour_attack")
local update_speed_task
--Suggestion: Add a pingable timer to the player's HUD for boss' cooldowns. --Unfortunately, that doesn't sound very practical.
--Suggestion: Add an option to hide cooldowns for mobs that are hidden(Tentacles, mobs in dark)
...
if TUNING.xiaoyao("toggle") ~= 0 then
	TheInput:AddKeyUpHandler(TUNING.xiaoyao("toggle"),function()
		if not InGame() then return else
			...
			---_G.ThePlayer.components.talker:Say("Show Timer widget: "..tostring(show_widget))
		end
	end)
end
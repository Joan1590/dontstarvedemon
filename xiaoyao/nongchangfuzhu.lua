...
local season_defs = {"autumn","winter","spring","summer"}
...
local function adjustButtons(self)
	...
	local allign = {}
	local location = TUNING.xiaoyao("location") or 2
	...
	local buttonOne, buttonTwo = self.buttonRegister, self.buttonNutrient
	if buttonOne == nil then buttonOne, buttonTwo = buttonTwo, nil end
	if buttonOne then
		buttonOne:SetPosition(x + buttonOne.width*allign[1]/2,
					    y + buttonOne.height*allign[2]/2, 0)
	end
	if buttonTwo then
		buttonTwo:SetPosition(x2+ buttonTwo.width*allign[1]/2,
					    y2+ buttonTwo.height*allign[2]/2, 0)
	end

	x = -((#nutrient_buttons-1) * sizeb)/2
	y = -2*height + TUNING.xiaoyao("groundstatusoffset")
	allign = {0,1}
	for i = 1,#nutrient_buttons do
		local display = self["display"..nutrient_buttons[i]]
		if display then
			display:SetPosition(x + display.width*allign[1]/2,
						  y + display.height*allign[2]/2, 0)
		end
		x = x + sizeb
	end
end
...
if TUNING.xiaoyao("keyregister") and TUNING.xiaoyao("keyregister") > 0 then
	TheInput:AddKeyDownHandler(TUNING.xiaoyao("keyregister"),function()
		if _G.TheFrontEnd and _G.ThePlayer and _G.TheFrontEnd:GetActiveScreen() == _G.ThePlayer.HUD.plantregistryscreen then
			_G.ThePlayer.HUD:ClosePlantRegistryScreen()
		elseif InGame() then
			_G.ThePlayer.HUD:OpenPlantRegistryScreen()
		end
	end)
end
if TUNING.xiaoyao("keyhud") and TUNING.xiaoyao("keyhud") > 0 then
	TheInput:AddKeyDownHandler(TUNING.xiaoyao("keyhud"),function()
		if InGame() and _G.TheWorld then
			_G.TheWorld:PushEvent("nutrientsvision",{enabled = not _G.ThePlayer.HUD.nutrientsover.shown})
		end
	end)
end
local function WidgetControlsPostInit(self)
	if TUNING.xiaoyao("buttonregister") then
		self.buttonRegister = self.top_root:AddChild(RegisterHUD())
	end
	if TUNING.xiaoyao("buttonhud") then
		self.buttonNutrient = self.top_root:AddChild(NutrientHUD())
	end
	for i = 1,#nutrient_buttons do
...
if TUNING.xiaoyao("knowallplants") then
	local function PlantRegistryPostInit(self)
		...
		if TUNING.xiaoyao("knowallplants") ~= "seeds" then
		...
		for k,plant in pairs(_G.TheWorld.Map:GetEntitiesOnTileAtPoint(pos.x,0,pos.z)) do
			if plant:HasTag("farm_plant") and not plant:HasTag("pickable") then
				if plant.plant_def and plant.plant_def.product then
					...
					local seasonMult = PLANT_DEFS[plant.plant_def.product].good_seasons
					seasonMult = type(seasonMult) == "table" and seasonMult[_G.TheWorld.state.season] and 1 or 0.5
					local stageMult  = StagesRemaining(plant)
					local values   = PLANT_DEFS[plant.plant_def.product].nutrient_consumption or {}
					local restores = PLANT_DEFS[plant.plant_def.product].nutrient_restoration or {}
					for i = 1,#values do
...
--[[
	...
	if nutrients then
		for i = 1,3 do
			...
			local affinities = {}
			if perStage[i] and perStage[i] < 0 and nutrients[i] == 0 then affinities[3] = 2 end
			widget:AddValues({perStage[i] or "-",total[i] or "-",nutrient_numbers[nutrients[i]+1] or "-"}, affinities)
	...
	local maxCount = TUNING.FARM_PANT_OVERCROWDING_MAX_PLANTS
	local num = plantcount == 0 and 0 or plantcount > maxCount and 5 or math.max(math.floor(plantcount*4/maxCount),1)
	widget:UpdateTextures(num == 0 and "Empty" or "Soil"..tostring(num))
	local text = tostring(plantcount).."/"..tostring(maxCount)
	widget:AddValues({" "..text},{plantcount > maxCount and 2 or 0})
	if ground then
	...
	widget:AddValues({water and "  "..string.format("%.1f",water*100.0).."%" or "-"},{water == 0.0 and perStage[1] and 2 or 0})
	if ground then
	...
	if item then
		if item.plant_def then
			...
			if product then
				...
				local seasons    = type(PLANT_DEFS[product].good_seasons) == "table" and PLANT_DEFS[product].good_seasons or {}
				local seasonMult = seasons[_G.TheWorld.state.season] and 1 or 0.5
				local values     = PLANT_DEFS[product].nutrient_consumption or {}
				local restores   = PLANT_DEFS[product].nutrient_restoration or {}
				...
				local affinities = {}
				local text = {}
				for k,v in pairs(season_defs) do
					text[k] = _S[v]
					affinities[k] = seasons[v] and 1 or 2
					if _G.TheWorld.state.season == v then text[k] = "→ "..text[k].." ←" end
				end
				text[#season_defs + 1] = ""
--				text[#season_defs + 1] = 1
--				affinities[#season_defs + 1] = 1
				local widget = _G.ThePlayer.HUD.controls.displaySoil
				widget:AddChangeValues(text,affinities,true)
				return
			end
		end
...
local function UpdateCropColor(inst, invert)
	...
	elseif inst:HasTag("farm_plant_killjoy") then
		state = "killjoy"
...
AddPrefabPostInitAny(function(inst)
	if inst == _G.TheWorld then
		inst:ListenForEvent("nutrientsvision", function(owner, data)
			if _G.ThePlayer then
				inst:DoTaskInTime(0, function() -- Add a small duration until everything shows up
					-- This might be annoying but the event gets called before values get changed
					if TUNING.xiaoyao("plantcolors") then
						local pos = _G.ThePlayer:GetPosition()
						for k,v in pairs(TheSim:FindEntities(pos.x,0,pos.z,40,nil,nil,{"farm_plant","farm_debris","heavy"})) do
							if v:HasTag("show_spoilage") == v:HasTag("heavy") then
								UpdateCropColor(v)
							end
						end
					end
					if TUNING.xiaoyao("groundstatus") then
						UpdateNutrientsInfo()
						if _G.ThePlayer.HUD.nutrientsover.shown then
	...
	elseif inst:HasTag("farm_plant") or inst:HasTag("farm_debris") or (inst:HasTag("show_spoilage") and inst:HasTag("heavy")) then
		-- Note: For some reason, picked Giant Crops don't have the "weighable_oversizedveggies" tag as soon as they spawn
		if TUNING.xiaoyao("plantcolors") and inst.plantupdate == nil then
...
AddPlayerPostInit(function(inst)
	inst:DoTaskInTime(1, function()
		...
		if TUNING.xiaoyao("nutrientsoveralpha") and inst.HUD and inst.HUD.nutrientsover and inst.HUD.nutrientsover.bg then
			inst.HUD.nutrientsover.bg:SetTint(1,1,1,TUNING.xiaoyao("nutrientsoveralpha")/100)
		end
	end)
end)
...
TUNING.ACTION_QUEUE_DEBUG_MODE = TUNING.xiaoyao("debug_mode")
...
local turf_grid_radius = TUNING.xiaoyao("turf_grid_radius")
local turf_grid_color = PLAYERCOLOURS[TUNING.xiaoyao("turf_grid_color")]
...
TheInput:AddKeyUpHandler(GetKeyFromConfig("auto_collect_key"), function()
    ...
    ThePlayer.components.talker:Say("自动收集: "..tostring(ActionQueuer.auto_collect))
end)
...
TheInput:AddKeyUpHandler(GetKeyFromConfig("endless_deploy_key"), function()
    ...
    ThePlayer.components.talker:Say("无休止的部署: "..tostring(ActionQueuer.endless_deploy))
end)
...
TheInput:AddKeyUpHandler(GetKeyFromConfig("last_recipe_key"), function()
    ...
    if not last_recipe then
        ThePlayer.components.talker:Say("找不到以前的配方")
        return
    end
    ...
    ThePlayer.components.talker:Say("制作最后的配方: "..last_recipe_name)
end)
...
local function ActionQueuerInit()
    ...
    ActionQueuer.double_click_speed = TUNING.xiaoyao("double_click_speed")
    ActionQueuer.double_click_range = TUNING.xiaoyao("double_click_range")
    ActionQueuer.deploy_on_grid = TUNING.xiaoyao("deploy_on_grid")
    ActionQueuer.auto_collect = TUNING.xiaoyao("auto_collect")
    ActionQueuer.endless_deploy = TUNING.xiaoyao("endless_deploy")
    ActionQueuer:SetToothTrapSpacing(TUNING.xiaoyao("tooth_trap_spacing"))
    ActionQueuer:SetFarmGrid(TUNING.xiaoyao("farm_grid")) -- 201221 null: added support for changing farm grids (3x3, 4x4)
    ActionQueuer:SetDoubleSnake(TUNING.xiaoyao("double_snake")) -- 210127 null: added support for snaking within snaking
    local r, g, b = unpack(PLAYERCOLOURS[TUNING.xiaoyao("selection_color")])
    ActionQueuer:SetSelectionColor(r, g, b, TUNING.xiaoyao("selection_opacity"))
end
...
local always_clear_queue = TUNING.xiaoyao("always_clear_queue")

...
local LANGUAGE = TUNING.xiaoyao("语言检测")
...
for k,v in pairs(ANNOUNCE_STRINGS) do
	if k ~= "UNKNOWN" and k ~= "_" and TUNING.xiaoyao(k) == false then
...
if HAS_MOD.COMBINED_STATUS then
	if TUNING.xiaoyao("OVERRIDEB") then
		-- Only try to do temperature if they have it configured to show temperature
		OVERRIDEB = GLOBAL.TUNING.xiaoyao("SHOWTEMPERATURE", HAS_MOD.COMBINED_STATUS)
	end
	if TUNING.xiaoyao("OVERRIDESELECT") then
		-- Only try to do temperature if they have it configured to show temperature
		OVERRIDESELECT = GLOBAL.TUNING.xiaoyao("SEASONOPTIONS", HAS_MOD.COMBINED_STATUS) ~= ""
	end
end
StatusAnnouncer:SetLocalParameter("WHISPER", TUNING.xiaoyao("WHISPER"))
StatusAnnouncer:SetLocalParameter("WHISPER_ONLY", HAS_MOD.WHISPER_ONLY)
StatusAnnouncer:SetLocalParameter("EXPLICIT", TUNING.xiaoyao("EXPLICIT"))
StatusAnnouncer:SetLocalParameter("OVERRIDEB", OVERRIDEB)
StatusAnnouncer:SetLocalParameter("OVERRIDESELECT", OVERRIDESELECT)
StatusAnnouncer:SetLocalParameter("SHOWDURABILITY", TUNING.xiaoyao("SHOWDURABILITY"))
StatusAnnouncer:SetLocalParameter("SHOWPROTOTYPER", TUNING.xiaoyao("SHOWPROTOTYPER"))
StatusAnnouncer:SetLocalParameter("SHOWEMOJI", TUNING.xiaoyao("SHOWEMOJI"))
	...
	if OVERRIDESELECT then
		...
		self._override_select_task = self.inst:DoPeriodicTask(0, function()
			...
			if season then --If Combined Status has loaded, add the button hint
				...
				season.announce_text:Hide()
				--We succeeded, clear the task
				if self._override_select_task then 
...
local _G = GLOBAL
_G.SHANGGAOGI = {
    DATA_FILE = "mod_config_data/shang_announce_data_save",
	DATA = {},
	Speech = { CantMake = {}, TryMake = {}, CantFind = {},},
    KeyDown = {},
}

local GI = _G.SHANGGAOGI
local S = _G.STRINGS._STATUS_ANNOUNCEMENTS
local _S = S._

GI.SaveData = function (filepath, data)
        ...
        function(load_success, str)
            if load_success == true then
                ...
                if success and string.len(str) > 0 then
                    ...
                    print ("[Key Items] Could not load "..filepath)
                    return {}
                end
            else
                print ("[Key Items] Can not find "..filepath)
                return {}
            end
        end
    )
...
local function Save()
    GI.SaveData(GI.DATA_FILE, GI.DATA)
end
GI.Save = Save
local function Load()
	GI.DATA = GI.LoadData(GI.DATA_FILE)
	local DATA = GI.DATA
	GI.STRINGS = S
	for ann1, a1 in pairs(DATA) do
		if a1 and type(a1) == "table" and ann1 == "_" then
			for ann2, a2 in pairs(a1) do
				if a2 and type(a2) == "table" and _S[ann2] then
					for ann3, a3 in pairs(a2) do
						if a3 and _S[ann2][ann3] then
							_S[ann2][ann3] = a3
						end
					end
				end
			end
		elseif a1 and type(a1) == "table" then
			for ann2, a2 in pairs(a1) do
				if a2 and type(a2) == "table" then
					for ann3, a3 in pairs(a2) do
						if type(a3) == "table" and S[ann1] and S[ann1][ann2] and S[ann1][ann2][ann3] then
							for ann4, a4 in pairs(a3) do
								if S[ann1][ann2][ann3][ann4] then
									S[ann1][ann2][ann3][ann4] = a4
								end
							end
						elseif a3 and S[ann1] and S[ann1][ann2] and S[ann1][ann2][ann3] then
							S[ann1][ann2][ann3] = a3
						end
					end
				end
...
-- SHANGGAOGI.Load 
GI.Load = Load
local function Init(ann1, ann2, ann3)
	if GI.DATA == nil then GI.DATA = {} end
	if GI.DATA["_"] == nil then GI.DATA["_"] = {} end
	if ann1 then
		if ann1 and not ann2 and not ann3 and GI.DATA["_"][ann1] == nil then
			GI.DATA["_"][ann1] = {}
		elseif ann1 then
			if GI.DATA[ann1] == nil then
				GI.DATA[ann1] = {}
			end
			if ann2 then
				if GI.DATA[ann1][ann2] == nil then
					GI.DATA[ann1][ann2] = {}
				end
				if ann3 then
					if GI.DATA[ann1][ann2][ann3] == nil then
						GI.DATA[ann1][ann2][ann3] = {}
					end
				end
			end
		end
...
GI.Init = Init
local function shangxuangao(strings, ann1, ann2, ann3, ann4)
	if strings == nil then return end
	local ann1 = ann1 or "ANNOUNCE_ITEM"
	local ann2 = ann2 or "I_HAVE"
	if ann4 and ann3 and S[ann1] and S[ann1][ann2] and S[ann1][ann2][ann3] and S[ann1][ann2][ann3][ann4] then
		S[ann1][ann2][ann3][ann4] = strings
		GI.Init(ann1, ann2, ann3)
		GI.DATA[ann1][ann2][ann3][ann4] = strings
		GI.Save()
		print("修改 {"..ann1.."."..ann2.."."..ann3.."."..ann4.."} 字符串为 {"..strings.."} 成功")
	elseif ann3 and S[ann1] and S[ann1][ann2] and S[ann1][ann2][ann3] then
		S[ann1][ann2][ann3] = strings
		GI.Init(ann1, ann2)
		GI.DATA[ann1][ann2][ann3] = strings
		GI.Save()
		print("修改 {"..ann1.."."..ann2.."."..ann3.."} 字符串为 {"..strings.."} 成功")
	elseif _S[ann1] and _S[ann1][ann2] then 
		_S[ann1][ann2] = strings
		GI.Init(ann1)
		GI.DATA["_"][ann1][ann2] = strings
		GI.Save()
		print("修改 {"..ann1.."."..ann2.."} 字符串为 {"..strings.."} 成功")
	else
		print("修改字符串 {"..strings.."} 失败，{"..ann1.."."..ann2..(ann3 and ("."..ann3) or "")"} 不存在")
	end
end
GI.XuanGao = shangxuangao
_G.shangxuangao = shangxuangao -- 修改指定字符
local function shangxuangaochongzhi()
	if GI.STRINGS then
		GI.DATA = {}
		_G.STRINGS._STATUS_ANNOUNCEMENTS = GI.STRINGS
		GI.Save()
		print("全部修改的字符串已被重置，需要重新加载，比如上下洞穴")
	else
		print("字符串重置失败")
	end
end
GI.ChongZhi = shangxuangaochongzhi
_G.shangxuangaochongzhi = shangxuangaochongzhi -- 重置所指定的修改字符
...
--[[
	示例：
		shangxuangao("朵朵的宝箱里有 ",nil,"WE_HAVE")
		shangxuangao"朵朵的口袋里有 "
		shangxuangao("朵朵做好了 ","ANNOUNCE_RECIPE","I_HAVE")
		shangxuangao("朵朵有 {ITEM} 发里福哨『{SKIN}』皮肤。","ANNOUNCE_SKIN","FORMAT_STRING")
		shangxuangao("朵朵的胃要爆炸了！","WOODIE","HUMAN","HUNGER","FULL")
		shangxuangao("朵朵的大脑在冒光！","WOODIE","HUMAN","SANITY","FULL")
]]
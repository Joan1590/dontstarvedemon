...
local IMAGETEXT = TUNING.xiaoyao("IMAGETEXT") or 2
local SHOWIMAGE = IMAGETEXT > 1
local SHOWTEXT = IMAGETEXT%2 == 1
local RIGHTSTICK = TUNING.xiaoyao("RIGHTSTICK")
--Backward-compatibility if they had changed the option
if TUNING.xiaoyao("LEFTSTICK") == false then RIGHTSTICK = true end
-- ONLYEIGHT isn't compatible with multiple rings; it will disable Party and Old emotes
local ONLYEIGHT = TUNING.xiaoyao("ONLYEIGHT")
local EIGHTS = {}
for i=1,8 do
	EIGHTS[i] = TUNING.xiaoyao("EIGHT"..i)
end
